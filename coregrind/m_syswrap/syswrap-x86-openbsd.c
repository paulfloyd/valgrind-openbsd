...
/*--------------------------------------------------------------------*/
/*--- Platform-specific syscalls stuff.      syswrap-x86-openbsd.c ---*/
/*--------------------------------------------------------------------*/
...
#include "priv_syswrap-openbsd.h"      /* for decls of linux-ish wrappers */
...
extern
Int do_syscall_clone_x86_openbsd ( Word (*fn)(void *),
                                   void* stack,
                                   Int   flags,
                                   void* arg,
                                   Int*  child_tid,
                         ...
                         UInt flags)
{
   ...
   */
   ...
   eax = do_syscall_clone_x86_openbsd(
            ML_(start_thread_NORETURN), stack, flags /*, &VG_(threads)[ctid], NULL*/ );
   res = VG_(mk_SysRes_x86_openbsd)( eax ); /* XXX edx returns too! */
...
PRE(sys_openbsd6_pread)
{
   *flags |= SfMayBlock;
   PRINT("sys_openbsd6_pread ( %" FMT_REGWORD "u, %#" FMT_REGWORD "x, %" FMT_REGWORD "u, %" FMT_REGWORD "u, %" FMT_REGWORD "u )", ARG1, ARG2, ARG3, ARG5, ARG6);
   PRE_REG_READ6(ssize_t, "pread",
                 unsigned int, fd, char *, buf, vki_size_t, count,
                 int, pad, unsigned int, off_low, unsigned int, off_high);

   if (!ML_(fd_allowed)(ARG1, "openbsd6_pread", tid, False))
      SET_STATUS_Failure( VKI_EBADF );
   else
      PRE_MEM_WRITE( "openbsd6_pread(buf)", ARG2, ARG3 );
}

POST(sys_openbsd6_pread)
{
   vg_assert(SUCCESS);
...
// openbsd6_pwrite 174
#if (FREEBSD_VERS <= FREEBSD_10)
PRE(sys_openbsd6_pwrite)
{
   Bool ok;
   *flags |= SfMayBlock;
   PRINT("sys_openbsd6_pwrite ( %" FMT_REGWORD "u, %#" FMT_REGWORD "x, %" FMT_REGWORD "u, %" FMT_REGWORD "u, %" FMT_REGWORD "u )", ARG1, ARG2, ARG3, ARG5, ARG6);
   PRE_REG_READ6(ssize_t, "openbsd6_pwrite",
                 unsigned int, fd, const char *, buf, vki_size_t, count,
                 int, pad, unsigned int, off_low, unsigned int, off_high);
   /* check to see if it is allowed.  If not, try for an exemption from
      --sim-hints=enable-outer (used for self hosting). */
   ok = ML_(fd_allowed)(ARG1, "openbsd6_pwrite", tid, False);
   if (!ok && ARG1 == 2/*stderr*/
   ...
   if (!ok)
      ...
      PRE_MEM_READ( "openbsd6_write(buf)", ARG2, ARG3 );
}
#endif

// SYS_openbsd6_mmap 197
#if (FREEBSD_VERS <= FREEBSD_10)
...
PRE(sys_openbsd6_mmap)
{
   SysRes r;

   PRINT("sys_openbsd6_mmap ( %#" FMT_REGWORD "x, %" FMT_REGWORD "u, %" FMT_REGWORD "u, %" FMT_REGWORD "u, %" FMT_REGWORD "u, pad%" FMT_REGWORD "u, lo0x%" FMT_REGWORD "x hi0x%" FMT_REGWORD "x)",
...
// openbsd6_lseek 199
#if (FREEBSD_VERS <= FREEBSD_10)
PRE(sys_openbsd6_lseek)
{
   PRINT("sys_openbsd6_lseek ( %" FMT_REGWORD "u, 0x%" FMT_REGWORD "x, 0x%" FMT_REGWORD "x, %" FMT_REGWORD "u )", ARG1,ARG3,ARG4,ARG5);
...
// openbsd6_truncate 200
#if (FREEBSD_VERS <= FREEBSD_10)
PRE(sys_openbsd6_truncate)
{
   *flags |= SfMayBlock;
...
// openbsd6_ftruncate 201
#if (FREEBSD_VERS <= FREEBSD_10)
PRE(sys_openbsd6_ftruncate)
{
   *flags |= SfMayBlock;
...
// See https://bugs.openbsd.org/bugzilla/show_bug.cgi?id=247386
...
PRE(sys_fake_sigreturn)
{
   ...
   SET_STATUS_from_SysRes( VG_(mk_SysRes_x86_openbsd)( tst->arch.vex.guest_EAX,
