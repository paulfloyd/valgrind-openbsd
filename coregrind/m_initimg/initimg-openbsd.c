...
static HChar** setup_client_env ( HChar** origenv, const HChar* toolname)
{
   ...
   HChar* preload_string     = VG_(malloc)("initimg-openbsd.sce.1",
                                           preload_string_len);
   /* Determine if there's a vgpreload_<tool>_<platform>.so file, and setup
      preload_string. */
   preload_tool_path = VG_(malloc)("initimg-openbsd.sce.2", preload_tool_path_len);
   ...
   ret = VG_(malloc) ("initimg-openbsd.sce.3",
   ...
   *(cpp + 1) = NULL;
   ...
   for (cpp = ret; cpp && *cpp; cpp++) {
      if (VG_(memcmp)(*cpp, ld_preload, ld_preload_len) == 0) {
         ...
         HChar *cp = VG_(malloc)("initimg-openbsd.sce.4", len);
   ...
   if (!ld_preload_done) {
      ...
      HChar *cp = VG_(malloc) ("initimg-openbsd.sce.5", len);
   ...
   if (VG_(is32on64)()) {
      for (cpp = ret; cpp && *cpp; cpp++) {
         if (VG_(memcmp)(*cpp, ld_32_preload, ld_32_preload_len) == 0) {
            Int len = VG_(strlen)(*cpp) + preload_string_len;
            HChar *cp = VG_(malloc)("initimg-openbsd.sce.4a", len);
            vg_assert(cp);
      ...
      if (!ld_32_preload_done) {
         ...
         HChar *cp = VG_(malloc) ("initimg-openbsd.sce.5a", len);
         vg_assert(cp);
                               ...
                               SizeT  clstack_max_size )
{
   ...
   struct auxv *auxv;
#if 1
   const struct auxv orig_auxv_dummy[] = {
      { .a_type = AT_PHDR },
      { .a_type = AT_PHENT },
      { .a_type = AT_PHNUM },
      { .a_type = AT_PAGESZ },
...
#else
   // XXX OpenBSD doesn't fill auxv for static binaries
   // XXX memcheck is not given auxv as statically linked
   // XXX Don't rely on orig_auxv but hard-code
   orig_auxv = orig_auxv_dummy;
#endif
   ...
   for (cauxv = orig_auxv; cauxv->a_type != VKI_AT_NULL; cauxv++) {
      ...
      case VKI_AT_PS_STRINGS:
         ...
         break;
#endif
#if (FREEBSD_VERS >= FREEBSD_13_1) && defined(VGO_freebsd)
      // case AT_FXRNG:
      // case AT_KPRELOAD:
#endif
      default:
...
#else
         vg_assert(resvn_size > VKI_PAGE_SIZE * 2);
         resvn_size -= VKI_PAGE_SIZE * 2;
         anon_start -= VKI_PAGE_SIZE * 2;
         anon_size += VKI_PAGE_SIZE * 2;
#endif
      }
...
#    endif
...
#if (FREEBSD_VERS >= FREEBSD_11) && defined(VGO_freebsd)
...
#if (FREEBSD_VERS >= FREEBSD_13_2) && defined(VGO_freebsd)
   ...
   *auxv = *orig_auxv;
   ...
   if (VG_(resolved_exename) == NULL) {
      VG_(resolved_exename) = VG_(strdup)("initimg-openbsd.sre.1", resolved_name);
   }
...
#else
   /* OpenBSD/amd64 expects RSP pointing to the top of stack exactly. */
   arch->vex.guest_RSP = iifii.initial_client_SP;
   /* OpenBSD/amd64 doesn't use RDI. */
#endif
