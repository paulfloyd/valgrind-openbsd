 ...
 * - OpenBSD ptrace works at the process level, not lwps
 ...
 * ptrace on OpenBSD so I doubt it can be fixed.
 *
 */
...
#else
   I_die_here : (sp) architecture missing in vgdb-invoker-openbsd.c
#endif
...
#else
      I_die_here : architecture missing in vgdb-invoker-openbsd.c
#endif
      }
