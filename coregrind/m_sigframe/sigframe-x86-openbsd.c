 ...
 * 0n          &_ctx_start
 *
 * Note that the 'struct sigframe' above is the one defined in OpenBSD.
 * 5 word members + ucontext + siginfo
 *
 * 'struct sigframe' below is the one defined in this file:
 *
 * Addr
 * OpenBSD struct sigfame
 * fpstate
 * vg_sigframe
 *
 */
                           ...
                           void *restorer)
{
   ...
   frame->sigNo = sigNo;
   frame->retaddr = (Addr)&VG_(x86_openbsd_SUBST_FOR_sigreturn);
